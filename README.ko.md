...
- `SPY`, `QQQ`, `VOO` 같은 미국 상장 ETF까지 지원하는 미국 주식/ETF 로컬 웹 대시보드: watchlist, 종목 검색, 캔들 차트, 신호 카드, 리스크 패널, AI 채팅.
- 선택적으로 Polygon/Massive BYOK 실시간 모드에서 `1s`, `5s`, `15s` 캔들, 체결창, Level 1 bid/ask spread를 볼 수 있습니다.
...
- 헤드앤숄더, 역헤드앤숄더, 불/베어 플래그, 페넌트, 삼각수렴, 쐐기, 쌍봉/쌍바닥 같은 차트 형태 패턴을 감지합니다.
...
기술적 지표와 차트 형태가 익숙하지 않다면 앱 실행 후 `/learn` 페이지를 열어보세요. EMA, VWAP, RSI, MACD, Bollinger Bands, ATR, 상대 거래량, OBV, 초단위 캔들, 체결창, bid/ask spread, Level 1 vs Level 2, 지지/저항, 하이킨아시, 손절, risk/reward, 헤드앤숄더, 플래그, 페넌트, 삼각수렴, 쐐기, 쌍봉/쌍바닥을 초보자 친화적으로 설명합니다.
...
기본 provider는 별도 키 없이 접근 가능한 공개 데이터를 사용합니다. 무료 데이터는 지연, 누락, rate limit, 제3자 약관 영향을 받을 수 있으므로 UI에서 데이터 출처와 시간을 직접 표시합니다. 초단위 캔들은 사용자 본인의 `POLYGON_API_KEY`가 필요하며, 실시간/지연 여부와 권한은 사용자의 Polygon/Massive 플랜에 따릅니다. 새로운 provider adapter도 출처/신선도 메타데이터를 유지하고 API 키를 브라우저에 노출하지 않아야 합니다.
